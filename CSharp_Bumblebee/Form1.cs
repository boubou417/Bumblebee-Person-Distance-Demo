...
namespace CSharp_Bumblebee
{
    public partial class Form1 : Form
    {
        ...
        private void GetStereoParameter(
            StereoCameraParameters parameters,
            INodeMap nodeMap)
        {
            parameters.coordinateOffset =
                (float)nodeMap.GetNode<IFloat>("Scan3dCoordinateOffset").Value;
            parameters.baseline =
                (float)nodeMap.GetNode<IFloat>("Scan3dBaseline").Value;
            parameters.focalLength =
                (float)nodeMap.GetNode<IFloat>("Scan3dFocalLength").Value;
            parameters.principalPointU =
                (float)nodeMap.GetNode<IFloat>("Scan3dPrincipalPointU").Value;
            parameters.principalPointV =
                (float)nodeMap.GetNode<IFloat>("Scan3dPrincipalPointV").Value;
            parameters.disparityScaleFactor =
                (float)nodeMap.GetNode<IFloat>("Scan3dCoordinateScale").Value;
            parameters.invalidDataFlag =
                nodeMap.GetNode<IBool>("Scan3dInvalidDataFlag").Value;
            parameters.invalidDataValue =
                (float)nodeMap.GetNode<IFloat>("Scan3dInvalidDataValue").Value;
        }

        private void SetBufferHandingMode(INodeMap nodeMap)
        {
            nodeMap.GetNode<IEnum>("StreamBufferHandlingMode").Value =
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            // This exhibition PC has no NVIDIA GPU.
            // Use the OpenCV CPU backend explicitly to avoid CUDA dependencies
            // and the Emgu.CV.Backend / Emgu.CV.Dnn.Backend ambiguity.
            ...
            try
            {
                while (capImg)
                {
                    ...
                    IManagedImage rectifiedOrigin = imageList.GetByPayloadType(
                        ImagePayloadType.IMAGE_PAYLOAD_TYPE_RECTIFIED_SENSOR1);
                    IManagedImage disparityOrigin = imageList.GetByPayloadType(
                        ImagePayloadType.IMAGE_PAYLOAD_TYPE_DISPARITY_SENSOR1);

                    if (rectifiedOrigin == null ||
                        disparityOrigin == null ||
                        rectifiedOrigin.IsIncomplete ||
                        disparityOrigin.IsIncomplete)
                    {
                        imageList.Release();
                        continue;
                    }
        ...
        {
            scale = Math.Min(
                YoloSize / (float)source.Width,
                YoloSize / (float)source.Height);
        ...
        unsafe private void PersonDetect(
            Net net,
            ushort* disparityData,
            ref Mat mat,
            ref Mat disparityMat)
        {
            float scale;
            int padX;
            int padY;
        ...
        unsafe private double ComputeZvalue(
            ...
            int imgHeight)
        {
            ...
            // Use the center torso area instead of the whole person box.
            // This reduces background disparity contamination and CPU work.
            ...
            Rectangle roi = ClampRect(
                new Rectangle(roiLeft, roiTop, roiWidth, roiHeight),
                imgWidth,
                imgHeight);

            List<double> distances = new List<double>(
                Math.Max(32, roi.Width * roi.Height / 16));

            // Sampling every second pixel greatly reduces CPU load while
            // preserving enough samples for a robust median distance.
            ...
            for (int y = roi.Top; y < roi.Bottom; y += sampleStep)
            {
                for (int x = roi.Left; x < roi.Right; x += sampleStep)
                {
                    ...
                    if (stereoCameraParameters.invalidDataFlag &&
                        Math.Abs(rawDisparity - stereoCameraParameters.invalidDataValue) < 0.5)
                    {
                        continue;
                    }

                    double actualDisparity =
                        rawDisparity * stereoCameraParameters.disparityScaleFactor +
