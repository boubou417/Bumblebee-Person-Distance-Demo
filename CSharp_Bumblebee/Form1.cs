...
namespace CSharp_Bumblebee
{
    public partial class Form1 : Form
    {
        ...
        {
            n.GetNode<IEnum>("StreamBufferHandlingMode").Value = StreamBufferHandlingModeEnum.NewestOnly.ToString();
        }
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                while (capImg)
                {
                    ...
                    IManagedImage ro = imageList.GetByPayloadType(ImagePayloadType.IMAGE_PAYLOAD_TYPE_RECTIFIED_SENSOR1);
                    IManagedImage d = imageList.GetByPayloadType(ImagePayloadType.IMAGE_PAYLOAD_TYPE_DISPARITY_SENSOR1);
                    ...
                    using (Mat rgb = new Mat(h, w, DepthType.Cv8U, 3, rectifiedImg.DataPtr, 0))
                    {
                        ...
                        {
                            pBox.BeginInvoke(new InvokeDelegate(InvokeDisplay), new Bitmap(bmp), w, h);
                        }
                    }
                }
            }
        ...
        {
            PoseKeypoint leftShoulder = person.Keypoints[5];
            PoseKeypoint rightShoulder = person.Keypoints[6];
            PoseKeypoint leftHip = person.Keypoints[11];
            PoseKeypoint rightHip = person.Keypoints[12];

            bool ls = IsValidKeypoint(leftShoulder, imgWidth, imgHeight);
            bool rs = IsValidKeypoint(rightShoulder, imgWidth, imgHeight);
            bool lh = IsValidKeypoint(leftHip, imgWidth, imgHeight);
            bool rh = IsValidKeypoint(rightHip, imgWidth, imgHeight);
            ...
            if (ls && rs)
            {
                ...
                return ClampPoint(new Point(
                    (int)Math.Round(shoulderX),
                    (int)Math.Round(shoulderY + person.Box.Height * 0.18f)), imgWidth, imgHeight);
            }

            return ClampPoint(new Point(
                person.Box.X + person.Box.Width / 2,
                person.Box.Y + person.Box.Height / 2), imgWidth, imgHeight);
        }

        private Point ClampPoint(Point point, int width, int height)
        {
            return new Point(
                Math.Max(0, Math.Min(width - 1, point.X)),
                Math.Max(0, Math.Min(height - 1, point.Y)));
        }
        ...
        private void DrawSkeleton(Mat mat, PoseKeypoint[] keypoints)
        {
            ...
            foreach (PoseKeypoint p in keypoints)
            {
                if (IsValidKeypoint(p, mat.Width, mat.Height))
                    CvInvoke.Circle(mat, new Point((int)p.X, (int)p.Y), circleSize, new MCvScalar(0, 255, 255), -1);
            }
        }

        private bool IsValidKeypoint(PoseKeypoint p, int w, int h)
        {
            return p.Confidence >= KeypointThreshold && p.X >= 0 && p.X < w && p.Y >= 0 && p.Y < h;
        }
        ...
        private Rectangle ClampRect(Rectangle r, int width, int height)
        {
            ...
            return rr > l && bb > t ? Rectangle.FromLTRB(l, t, rr, bb) : Rectangle.Empty;
        }
        ...
        private unsafe double ComputeZvalue(Point center, Rectangle personRect, ushort* disparityData, int imgWidth, int imgHeight)
        {
            ...
            for (int y = roi.Top; y < roi.Bottom; y += 2)
            {
                for (int x = roi.Left; x < roi.Right; x += 2)
                {
                    ...
                    if (raw == 0)
                        continue;
                    if (stereoCameraParameters.invalidDataFlag && Math.Abs(raw - stereoCameraParameters.invalidDataValue) < .5)
                        continue;

                    double disp = raw * stereoCameraParameters.disparityScaleFactor + stereoCameraParameters.coordinateOffset;
                    if (disp <= 0)
                        continue;
        ...
        private double UpdateSmoothedDistance(Point center, double rawDistance)
        {
            ...
            double alpha = difference > DistanceJumpThresholdMeters ? 0.55 : DistanceEmaAlpha;
            bestTrack.SmoothedDistance = bestTrack.SmoothedDistance * (1.0 - alpha) + rawDistance * alpha;
            return bestTrack.SmoothedDistance;
        }
        ...
        {
            distanceTracks.RemoveAll(track => track.MissedFrames > DistanceTrackMaxMissedFrames);
        }
