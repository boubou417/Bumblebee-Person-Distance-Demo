...
namespace CSharp_Bumblebee
{
    ...
    {
        [DllImport("kernel32", SetLastError = true, CharSet = CharSet.Auto)] private static extern bool SetDllDirectory(string dllPathName);
        [DllImport("user32.dll")] public static extern bool ReleaseCapture();
        [DllImport("user32.dll")] public static extern int SendMessage(IntPtr hWnd, int Msg, int wParam, int lParam);
        [DllImport("Gdi32.dll", EntryPoint = "CreateRoundRectRgn")] private static extern IntPtr CreateRoundRectRgn(int l, int t, int r, int b, int ew, int eh);
        ...
        private int nextDistanceTrackId = 1; private int poseFrameCounter;
        ...
        private void startBtn_Click(object sender,EventArgs e){if(!started){distanceTracks.Clear();cachedPosePeople.Clear();nextDistanceTrackId=1;poseFrameCounter=0;capImg=true;capImgComplete=false;cam.BeginAcquisition();backgroundWorker1.RunWorkerAsync();startBtn.Text="停止取像";connectBtn.Enabled=false;started=true;}else{capImg=false;startBtn.Enabled=false;}}
        ...
        private void backgroundWorker1_DoWork(object sender,DoWorkEventArgs e)
        {
            ...
            IManagedImageList imageList=new ManagedImageList();IManagedImage rectifiedImg=new ManagedImage();IManagedImage disparityImg=new ManagedImage();Stopwatch sw=new Stopwatch();
            try
            {
                while(capImg)
                {
                    sw.Restart(); imageList=cam.GetNextImageSync(3000); sw.Stop(); SetCaptureMs(sw.Elapsed.TotalMilliseconds);
                    IManagedImage ro=imageList.GetByPayloadType(ImagePayloadType.IMAGE_PAYLOAD_TYPE_RECTIFIED_SENSOR1);IManagedImage d=imageList.GetByPayloadType(ImagePayloadType.IMAGE_PAYLOAD_TYPE_DISPARITY_SENSOR1);
                    if(ro==null||d==null||ro.IsIncomplete||d.IsIncomplete){imageList.Release();continue;}
                    sw.Restart();rectifiedImg.DeepCopy(ro);disparityImg.DeepCopy(d);ro.Dispose();d.Dispose();imageList.Release();sw.Stop();SetCopyMs(sw.Elapsed.TotalMilliseconds);
                    ...
                    {
                        sw.Restart();CvInvoke.CvtColor(rgb,bgr,ColorConversion.Rgb2Bgr);sw.Stop();SetConvertMs(sw.Elapsed.TotalMilliseconds);
                        unsafe
                        {
                            bool runInference=cachedPosePeople.Count==0||(poseFrameCounter%PoseInferenceInterval)==0;
                            if(runInference)DetectPose(net,bgr);
                            sw.Restart();DrawCachedPoseAndDistance((ushort*)disparityImg.NativeData,bgr);sw.Stop();SetDistanceRenderMs(sw.Elapsed.TotalMilliseconds);
                        }
                        poseFrameCounter++;
                        sw.Restart();using(Bitmap bmp=new Bitmap(w,h,stride,PixelFormat.Format24bppRgb,bgr.DataPointer)){pBox.BeginInvoke(new InvokeDelegate(InvokeDisplay),new Bitmap(bmp),w,h);}sw.Stop();SetBitmapQueueMs(sw.Elapsed.TotalMilliseconds);
                    }
                }
            }
            finally{net.Dispose();rectifiedImg.Dispose();disparityImg.Dispose();distanceTracks.Clear();cachedPosePeople.Clear();capImgComplete=true;BeginInvoke(new Action(()=>{try{cam.EndAcquisition();}catch{}startBtn.Text="開始取像";startBtn.Enabled=true;connectBtn.Enabled=true;started=false;}));}
        }
        ...
        {
            Stopwatch total=Stopwatch.StartNew();float scale;int padX,padY;
            ...
            {
                net.SetInput(blob);Stopwatch forward=Stopwatch.StartNew();Mat output=net.Forward();forward.Stop();SetInferenceMs(forward.Elapsed.TotalMilliseconds);Stopwatch post=Stopwatch.StartNew();
                using(output)using(Mat reshaped=output.Reshape(1,PoseOutputChannels))using(Mat transposed=new Mat())
                {
                    ...
                    for(int i=0;i<rows;i++,data+=cols){float score=data[4];if(score<ConfidenceThreshold)continue;float cx=(data[0]-padX)/scale,cy=(data[1]-padY)/scale,bw=data[2]/scale,bh=data[3]/scale;Rectangle box=ClampRect(new Rectangle((int)(cx-bw/2),(int)(cy-bh/2),(int)bw,(int)bh),mat.Width,mat.Height);if(box.IsEmpty)continue;PosePerson person=new PosePerson{Box=box,Keypoints=new PoseKeypoint[KeypointCount]};for(int k=0;k<KeypointCount;k++){int o=5+k*3;person.Keypoints[k]=new PoseKeypoint{X=(data[o]-padX)/scale,Y=(data[o+1]-padY)/scale,Confidence=data[o+2]};}people.Add(person);boxes.Add(box);scores.Add(score);}
                    cachedPosePeople.Clear();if(people.Count>0){using(VectorOfRect bv=new VectorOfRect(boxes.ToArray()))using(VectorOfFloat sv=new VectorOfFloat(scores.ToArray()))using(VectorOfInt indices=new VectorOfInt()){DnnInvoke.NMSBoxes(bv,sv,ConfidenceThreshold,NmsThreshold,indices);foreach(int index in indices.ToArray())cachedPosePeople.Add(people[index]);}}
                }
                post.Stop();SetPosePostMs(post.Elapsed.TotalMilliseconds);
            }
            total.Stop();
        }

        private unsafe void DrawCachedPoseAndDistance(ushort* disparityData,Mat mat){AgeDistanceTracks();foreach(PosePerson person in cachedPosePeople){DrawSkeleton(mat,person.Keypoints);Point center=GetTorsoCenter(person,mat.Width,mat.Height);double rawDistance=ComputeZvalue(center,person.Box,disparityData,mat.Width,mat.Height);double displayDistance=UpdateSmoothedDistance(center,rawDistance);CvInvoke.Circle(mat,center,circleSize+1,new MCvScalar(0,255,255),-1);if(displayDistance>0)CvInvoke.PutText(mat,displayDistance.ToString("F2")+"m",new Point(center.X+8,center.Y-8),FontFace.HersheyTriplex,fontSize,new MCvScalar(255,255,255),fontThick);}RemoveExpiredDistanceTracks();}
        private Point GetTorsoCenter(PosePerson person,int imgWidth,int imgHeight){PoseKeypoint lsP=person.Keypoints[5],rsP=person.Keypoints[6],lhP=person.Keypoints[11],rhP=person.Keypoints[12];bool ls=IsValidKeypoint(lsP,imgWidth,imgHeight),rs=IsValidKeypoint(rsP,imgWidth,imgHeight),lh=IsValidKeypoint(lhP,imgWidth,imgHeight),rh=IsValidKeypoint(rhP,imgWidth,imgHeight);if(ls&&rs&&lh&&rh)return ClampPoint(new Point((int)Math.Round((lsP.X+rsP.X+lhP.X+rhP.X)/4.0),(int)Math.Round((lsP.Y+rsP.Y+lhP.Y+rhP.Y)/4.0)),imgWidth,imgHeight);if(ls&&rs)return ClampPoint(new Point((int)Math.Round((lsP.X+rsP.X)*.5f),(int)Math.Round((lsP.Y+rsP.Y)*.5f+person.Box.Height*.18f)),imgWidth,imgHeight);return ClampPoint(new Point(person.Box.X+person.Box.Width/2,person.Box.Y+person.Box.Height/2),imgWidth,imgHeight);}
        private Point ClampPoint(Point p,int w,int h){return new Point(Math.Max(0,Math.Min(w-1,p.X)),Math.Max(0,Math.Min(h-1,p.Y)));}
        private void DrawSkeleton(Mat mat,PoseKeypoint[] keypoints){for(int i=0;i<SkeletonEdges.GetLength(0);i++){PoseKeypoint a=keypoints[SkeletonEdges[i,0]],b=keypoints[SkeletonEdges[i,1]];if(IsValidKeypoint(a,mat.Width,mat.Height)&&IsValidKeypoint(b,mat.Width,mat.Height))CvInvoke.Line(mat,new Point((int)a.X,(int)a.Y),new Point((int)b.X,(int)b.Y),new MCvScalar(0,255,0),2);}foreach(PoseKeypoint p in keypoints)if(IsValidKeypoint(p,mat.Width,mat.Height))CvInvoke.Circle(mat,new Point((int)p.X,(int)p.Y),circleSize,new MCvScalar(0,255,255),-1);}
        ...
        private unsafe double ComputeZvalue(Point center,Rectangle personRect,ushort* disparityData,int imgWidth,int imgHeight){Rectangle person=ClampRect(personRect,imgWidth,imgHeight);if(person.IsEmpty)return 0;int rw=Math.Max(12,(int)(person.Width*.22)),rh=Math.Max(12,(int)(person.Height*.18));Rectangle roi=ClampRect(new Rectangle(center.X-rw/2,center.Y-rh/2,rw,rh),imgWidth,imgHeight);if(roi.IsEmpty)return 0;List<double> distances=new List<double>(Math.Max(32,roi.Width*roi.Height/8));for(int y=roi.Top;y<roi.Bottom;y+=2)for(int x=roi.Left;x<roi.Right;x+=2){ushort raw=disparityData[y*imgWidth+x];if(raw==0)continue;if(stereoCameraParameters.invalidDataFlag&&Math.Abs(raw-stereoCameraParameters.invalidDataValue)<.5)continue;double disp=raw*stereoCameraParameters.disparityScaleFactor+stereoCameraParameters.coordinateOffset;if(disp<=0)continue;double distance=stereoCameraParameters.focalLength*stereoCameraParameters.baseline/disp;if(distance>0&&distance<100000)distances.Add(distance);}return GetMedianInPlace(distances);}
        private double UpdateSmoothedDistance(Point center,double rawDistance){DistanceTrack bestTrack=null;double bestDistanceSquared=DistanceTrackMatchPixels*DistanceTrackMatchPixels;foreach(DistanceTrack track in distanceTracks){if(track.MatchedThisFrame)continue;double dx=track.Center.X-center.X,dy=track.Center.Y-center.Y,ds=dx*dx+dy*dy;if(ds<bestDistanceSquared){bestDistanceSquared=ds;bestTrack=track;}}if(bestTrack==null){bestTrack=new DistanceTrack{Id=nextDistanceTrackId++,Center=center,SmoothedDistance=rawDistance,HasDistance=rawDistance>0,MissedFrames=0,MatchedThisFrame=true};distanceTracks.Add(bestTrack);return bestTrack.HasDistance?bestTrack.SmoothedDistance:0;}bestTrack.Center=center;bestTrack.MissedFrames=0;bestTrack.MatchedThisFrame=true;if(rawDistance<=0)return bestTrack.HasDistance?bestTrack.SmoothedDistance:0;if(!bestTrack.HasDistance){bestTrack.SmoothedDistance=rawDistance;bestTrack.HasDistance=true;return bestTrack.SmoothedDistance;}double difference=Math.Abs(rawDistance-bestTrack.SmoothedDistance);double alpha=difference>DistanceJumpThresholdMeters?.55:DistanceEmaAlpha;bestTrack.SmoothedDistance=bestTrack.SmoothedDistance*(1-alpha)+rawDistance*alpha;return bestTrack.SmoothedDistance;}
        private void AgeDistanceTracks(){foreach(DistanceTrack track in distanceTracks){track.MatchedThisFrame=false;track.MissedFrames++;}}
        private void RemoveExpiredDistanceTracks(){distanceTracks.RemoveAll(track=>track.MissedFrames>DistanceTrackMaxMissedFrames);}
        ...
        private void Form1_FormClosing(object sender,FormClosingEventArgs e){capImg=false;DisposePerformanceOverlay();try{if(started)cam.EndAcquisition();}catch{}try{if(connected)cam.DeInit();}catch{}cam?.Dispose();}
    ...
    internal class DistanceTrack{public int Id;public Point Center;public double SmoothedDistance;public bool HasDistance;public int MissedFrames;public bool MatchedThisFrame;}
}
